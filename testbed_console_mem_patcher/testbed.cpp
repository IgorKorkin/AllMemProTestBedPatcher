		...
		if (GetTempPath(MAX_PATH, tmp_path)) {
			...
			b_res =
				((0 == wcscat_s(lpTempFileName, MAX_PATH, tmp_path)) &&
					(0 == wcscat_s(lpTempFileName, MAX_PATH, TESTBEDMP_SYS_FILE)));
...
namespace testbed_for_exploitation {
	...
	bool TestBed::load_driver_from_file(TCHAR * binFile) {
		auto b_res = false;
		service_manager.set_names(TESTBEDMP_SERVNAME_APP, binFile);
		service_manager.stop_driver();
		service_manager.remove_driver();
		if (service_manager.add_driver()) {
			if (service_manager.start_driver()) {
				if (INVALID_HANDLE_VALUE != service_manager.open_device(TESTBEDMP_LINKNAME_APP)) {
					b_res = true;
					print::print_mes(L"The [%s] driver is active!", TESTBEDMP_SERVNAME_APP);
					...
					print::print_last_err(L"err open device %s ", TESTBEDMP_LINKNAME_APP);
				}
			}
			else {
				print::print_last_err(L"err start driver %s %s", TESTBEDMP_SERVNAME_APP, binFile);
				service_manager.stop_driver();
				service_manager.remove_driver();
			}
		}
		else {
			print::print_last_err(L"err add driver %s %s", TESTBEDMP_SERVNAME_APP, binFile);
			service_manager.stop_driver();
			service_manager.remove_driver();
		}
	...
	/*  */
	bool TestBed::hide_proc(ULONG64 procId) {

		bool b_res =
			service_manager.send_ctrl_code(TESTBED_MEM_PATCHER_HIDE_PROC, (LPVOID)&procId, sizeof ULONG64, NULL, 0, 0);

		return b_res;
	}

	/*  */
	bool TestBed::read_1byte(ULONG64 addr) {

		ADDR_BYTE addr_byte = { addr, 0 };

		bool b_res =
			service_manager.send_ctrl_code(TESTBED_MEM_PATCHER_READ_1_BYTE, (LPVOID)&addr_byte, sizeof ADDR_BYTE, NULL, 0, 0);
	...
	/*  */
	bool TestBed::write_1byte(ULONG64 addr, char value){

		ADDR_BYTE addr_byte = { addr, value };

		bool b_res = 
			service_manager.send_ctrl_code(TESTBED_MEM_PATCHER_WRITE_1_BYTE, (LPVOID)&addr_byte, sizeof ADDR_BYTE, NULL, 0, 0);
		if (b_res) {
	...
	/*  */
	bool TestBed::write_8bytes(ULONG64 addr, ULONG64 value){

		ADDR_8BYTES addr_byte = { addr, value };

		bool b_res =
			service_manager.send_ctrl_code(TESTBED_MEM_PATCHER_WRITE_8_BYTES, (LPVOID)&addr_byte, sizeof ADDR_8BYTES, NULL, 0, 0);
